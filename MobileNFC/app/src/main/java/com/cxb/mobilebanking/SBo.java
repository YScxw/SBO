...
public class SBo extends Activity implements Callback
{
	...
	private VoicePlayer player;//声波播放器
     ...
     //end add by yywang    
		...
		replaysound.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				// TODO Auto-generated method stub
				//chirpService.sendDataToken(dataToken);
				Log.i(TAG, "send yym data is:"+dataToken);
				String encodeData = DataEncoder.encodeString(dataToken);
...
//				valueViewTip.setText(strMsgToShow);
				 ...
				 intent.setClass(SBo.this, SelectPage.class);               
				 SBo.this.startActivity(intent);
				//end add 
			}
		});
		// 处理扫一扫取款
		sweep.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				valueViewTip.setText("请使用手机扫描ATM上的二维码图片！或操作ATM发出声音");
				valueItems.clear();
				aa.notifyDataSetChanged();
				showViewHome();
				reStartCamera();

...
//				valueViewTip.setText(strMsgToShow);
				 ...
				 intent.setClass(CaptureActivity.this, NFCActivity.class);               
		...
		sweep.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				valueViewTip.setText("请使用手机扫描ATM上的二维码图片");
				valueItems.clear();
				aa.notifyDataSetChanged();
				showViewHome();
				reStartCamera();
	...
	public void onRecv(String dataToken) {
		if (operationType[2] == 1) {
			Log.i(TAG, "onRecv:" + dataToken);
			Toast.makeText(this, "Received successfully", Toast.LENGTH_LONG)
					.show();
			if (true) {
				valueViewTip.setText("已采集到设备信息，系统正在为您处理");
				stopStartCamera();
				/*try {
					...
					try {
						byte[] tmp = dataToken.substring(0, 8).getBytes("utf-8");
						sha1.update(tmp);
						message = bytesToHexString(sha1.digest(tmp));
				...
				message = "tmsg2" + message.toUpperCase();*/
			...
			if (true) {
				...
				if(strGetValue.equals("100000000")){
					...
					valueViewTip.setText("匹配格式错误");
				}
			}
			else {
				Log.i(TAG, "onRecv:" + "接收的数据格式不对");
				Toast.makeText(this, "接收的数据格式不对，不处理", Toast.LENGTH_LONG).show();
			}
			...
			if (true) {
				...
				if(Integer.parseInt(strGetValue) == 3){
					...
					valueViewTip.setText(dataToken+"交换数据错误");
				}
			}
			else {
				Log.i(TAG, "onRecv:" + "接收的数据格式不对");
